...
function problemA() {
  ...
  // promise version
  let p1 = promisifiedReadFile("poem-two/stanza-01.txt").then((stranza1) =>
    blue(stranza1)
  );
  let p2 = promisifiedReadFile("poem-two/stanza-02.txt").then((stranza2) =>
    blue(stranza2)
  );
...
function problemB() {
  ...
  // promise version
  let promesasArr = filenames.map((e) => promisifiedReadFile(e).then((value) => blue(value)))

Promise.all(promesasArr).then(() => console.log("done"));
